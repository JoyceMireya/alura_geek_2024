...
export async function displayProducts(type, key) {
  ...
  if (type === "all") {
    ...
    addButtonToGallery(gallery, "adicionar-produto.html", "Agregar producto");
    ...
    productsResponse.forEach((category) => {
      ...
      const maxItemsMobile =
        quantity < MAX_ITEMS_MOBILE ? quantity : MAX_ITEMS_MOBILE;

      addTitleToGallery(gallery, category["categoryName"]);
      addLinkToGallery(
        gallery,
        `categoria.html?cat=${category["categoryId"]}`,
        "Ver todo &#10132;"
      );
    ...
    if (category != null) {
      ...
      addTitleToGallery(gallery, "La categoria espicificada no existe!");
    }
  } else if (type === "search") {
...
function createProductCard(product) {
  ...
  productCard.innerHTML = `
        <img src="${product.picture}" onclick="location.href='detalhe-produto.html?id=${product.id}'" alt="Foto de producto" class="item__image">
